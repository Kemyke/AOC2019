...
namespace AOC201915
{
    class Program
    {
        class Computer
        {
            ...
            public Tuple<long, bool> Compute(List<int> moves)
            {
                int currOpCode = 0;
                long output = 0;
                int ipnnum = 0;
                while (currOpCode != 99)
                {
                    ...
                    switch (currOpCode)
                    {
                        ...
                        case 3:
                            ...
                            program[GetIndex(paramModes[0], currPosition + 1)] = moves[ipnnum++];
                            currPosition += 2;
                            break;
                        case 4:
                            paramModes = GetModes(inst, 1);
        ...
        class Place
        {
            ...
            public int OStep { get; set; } = -1;
        }

        static List<Place> map = new List<Place>();

        static Place AfterMoves(List<int> moves)
        {
            int x = 0;
            int y = 0;
            ...
            var place = map.SingleOrDefault(p => p.X == x && p.Y == y);
            if (place == null)
            {
                place = new Place { X = x, Y = y, Dist = moves.Count, Type = "?" };
            }
            return place;
        }
        ...
        static void Main(string[] args)
        {
            ...
            while(moves.Any())
            {
                ...
                {
                    if (place.Type != "#" && place.Dist > cm.Count)
                    {
                        place.Dist = cm.Count;
                        cont = true;
                    }
                ...
                if(cont)
                {
                    moves.Enqueue(cm.Append(1).ToList());
                    moves.Enqueue(cm.Append(2).ToList());
                    moves.Enqueue(cm.Append(3).ToList());
                    moves.Enqueue(cm.Append(4).ToList());
                }
            }

            var h = map.Single(p => p.Type == "X");
            h.OStep = 0;
            ...
            //Computer aaa2 = new Computer(program.ToList());
            //var aaar2 = aaa2.Compute(new List<int> { 4, 3 });

            //program = omoves;
            //Computer aaa = new Computer(program.ToList());
            //var aaar = aaa.Compute(new List<int> { 4, 3 });

            moves.Enqueue(omoves2.Append(1).ToList());
            moves.Enqueue(omoves2.Append(2).ToList());
            moves.Enqueue(omoves2.Append(3).ToList());
            moves.Enqueue(omoves2.Append(4).ToList());
            ...
            while (moves.Any())
            {
                ...
                else
                {
                    if (place.Type != "#" && place.Dist > cm.Count)
                    {
                        place.Dist = cm.Count - omoves2.Count;
                        cont = true;
                    }
                }
            ...
            var f = map.Where(p=> p.Type == ".").Max(p => p.Dist);
            var ff = map.Where(p => p.Dist == f).ToList();
            var fx = ff.First().X;
            var fy = ff.First().Y;
            ...
            for (int j = miny; j <= maxy; j++)
            {
                ...
                {
                    var place = map.SingleOrDefault(p => p.X == i && p.Y == j);
                    if(place == null)
                    {
                        Console.Write("0");
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if(place.Dist<10)
                                {
                                    Console.Write(place.Dist.ToString());
                                }
                                else
                                Console.Write(place.Type);
                            }
                        }
                    }
                }
